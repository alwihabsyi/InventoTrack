...
import LaporanPermintaanTableRow from './laporan-permintaan-table-row';

export default function LaporanPermintaanItems({
    userId,
    namaUnit,
    namaAnggota,
    status
    ...
    const getStatus = () => {
        let newStatus = ""
        if (status === "Pending") {
            newStatus = "Diterima Ketua"
        } if (status === "Diterima") {
            newStatus = "Diterima Admin"
            ...
            <Scrollbar>
                ...
                <TableContainer sx={{ overflow: 'unset' }}>
                    <Table sx={{ minWidth: 600 }}>
                        ...
                        <TableBody>
                            {dataFiltered
                                .filter((row) => row.status === getStatus())
                                .map((row) => (
                                    <LaporanPermintaanTableRow
                                        key={row.id}
                                        id={row.id}
...
LaporanPermintaanItems.propTypes = {
